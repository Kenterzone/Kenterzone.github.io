...
function setup(){
	...
	document.getElementById("app_guid").innerHTML =`<li><a href="#" onClick="document.getElementById('v_channel').src ="https://player.twitch.tv/?channel=${channel_id}&parent=Kenterzone.github.io"">Home</a></li><li><a href="#" onClick="document.getElementById('v_channel').src ="https://player.twitch.tv/?channel=${channel_id}&parent=Kenterzone.github.io"">NHK&CDTV</a></li><BR/>${musicGuid.map(guidmap).join("")}<font color=Red>Tomorrow</font><BR/> ${musicGuid.map(guidtmrmap).join("")}`;
