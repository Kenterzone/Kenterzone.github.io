...
function setup(){
	...
	document.getElementById("app_guid").innerHTML =`<font color=white ><a href="#" text-decoration:none onClick="document.getElementById('player').src ='https://player.twitch.tv/?channel=${channel_id}&parent=Kenterzone.github.io'">Home</a> <a href="#" text-decoration:none onClick="document.getElementById('player').src ='https://player.twitch.tv/?channel=${channel_id2}&parent=Kenterzone.github.io'">NHK&CDTV</a></font><BR/>${musicGuid.map(guidmap).join("")}<font color=Red>Tomorrow</font><BR/> ${musicGuid.map(guidtmrmap).join("")}`;
